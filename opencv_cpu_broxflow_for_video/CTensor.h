...
public:
  ...
  // readFromOpencvMat
  void readFromOpencvMat(Mat& image);
...
//////////////////////////////////
template <class T>
void CTensor<T>::readFromOpencvMat(Mat& image){
  mXSize = image.cols;
  mYSize = image.rows;
  mZSize = image.channels();
  delete[] mData;
  mData = new T[mXSize*mYSize*mZSize];
  // Read image data
  int aSize = mXSize*mYSize;
...
//      mData[i*mXSize + j] = image.at<Vec2b>(i,j);
//      }
//     }
    }
  else {
    ...
    for (int i = 0; i < mYSize; i++) {
      for(int j = 0; j< mXSize; j++){
      mData[i*mXSize + j] = (float)image.at<Vec3b>(i,j)[2];
      mData[i*mXSize + j +aSize] = (float)image.at<Vec3b>(i,j)[1];
      mData[i*mXSize + j +aSizeTwice] = (float)image.at<Vec3b>(i,j)[0];
      }
    }
  }
}
