...
if __name__ == '__main__':
    ...
    jhmdb_test = JHMDB(args.imdb, 'TEST')
    roidb = jhmdb_test.roidb
    ...
    if not os.path.exists(args.savepath):
        ...
        pred_all_dets = {}
        n_fr = len(jhmdb_test.image_index)
        for i in range(n_fr):
            image_name = jhmdb_test.image_index[i]
            image_path = os.path.join(jhmdb_test._data_path, image_name)
    ...
    # ap_all = action.evaluate_frameAP(roidb, pred_all_dets, jhmdb_test._classes)
    # print jhmdb_test._classes[1:]
    # print ap_all
    # print 'mean frame AP: {}'.format(np.mean(np.array(ap_all)))

    # ==== video AP evaluation ====
    gt_video = jhmdb_test.get_test_video_annotations()
    ap_all = action.evaluate_videoAP(gt_video, pred_all_dets, jhmdb_test._classes, 0.2)
    print jhmdb_test._classes[1:]
    print ap_all
